...
在 `Milk-V Meles` 刷写系统中若使用了[最新 `RevyOS` 内核](https://fast-mirror.isrc.ac.cn/revyos/extra/images/meles/20250729/)（截止编写时为 6.6.82），开发板默认使用的是外置天线而非板载陶瓷天线。如果没有在同轴/IPEX 座上连接外置天线，会导致 Wi-Fi 信号很弱，甚至完全搜不到 Wi-Fi 信号。
