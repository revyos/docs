...
部分 SD 卡在开发板上可能出现兼容性问题，请在 [LicheePi 4A SD卡兼容清单](../../miscellaneous/lpi4a-sdcard-list/) 查找您的 SD 卡是否有已知的兼容性问题。
:::
...
请注意，注意解压后的文件大小约为**10.2GB**，解压时请注意本地存储空间是否足够！
...
uboot 文件和 boot 文件刷写较快，而 root 文件需要大约5分钟才能刷写完成。如果在刷写 root 文件时并非是30+的数据块，而是2000+或是3000+，那么证明前面的刷写操作有误，在此情况下写入完成后也无法启动镜像，请重新进行刷写操作。
:::
