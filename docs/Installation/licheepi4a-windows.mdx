...
LicheePi4A 目前支持两种启动方式，分别是[从SD card 启动](#从-sd-card-启动)和[从 eMMC 启动](#从-emmc-启动)，对于两种不同的启动方式，此教程中都有进行说明，请根据自己所需的刷写方式点击进行跳转查看。
...
在拔出 SD 卡前，请确保 BalenaEtcher 的烧录过程已完成，并且 SD 卡的指示灯不再闪烁，并在系统托盘中弹出了SD卡。否则可能会导致 SD 卡损坏或数据丢失。
:::
