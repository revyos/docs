---
title: Shufflenet
description: 在 RevyOS 系统上运行 Shufflenet 模型的教程
sidebar_position: 3
---
...
本教程将引导你如何通过 CPU 在 RevyOS 系统上运行 Shufflenet 模型。Shufflenet 是一种轻量级卷积神经网络，专为移动设备和嵌入式设备等计算资源受限场景设计。
...
适用于本教程的代码位于 `classification/shufflenet` 目录下。

## 模型获取

我们使用的模型来自 [ONNX 模型仓库](https://github.com/onnx/models/blob/main/validated/vision/classification/shufflenet/model/shufflenet-9.onnx)，可以通过以下命令下载 Shufflenet 模型：
...
在 `classification/shufflenet` 目录下，使用以下命令编译应用程序：
...
- -static：指定为静态链接
- -o shufflenet_example：指定生成名为 shufflenet_example 的可执行文件

编译命令正确执行完成后会在示例目录生成 `shufflenet_example` 文件
...
```shell-session
$ scp -r ../shufflenet/ debian@<开发板IP>:/home/debian/shufflenet/
```
...
进入开发板的 `/home/debian/shufflenet` 目录，确保你已经在 RevyOS 上安装了 SHL 库并配置了 `LD_LIBRARY_PATH`。然后运行以下命令：
