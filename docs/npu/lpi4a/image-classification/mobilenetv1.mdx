---
title: MobilenetV1
description: 在 RevyOS 系统上运行 MobilenetV1 模型的教程
sidebar_position: 6
---
...
本教程将引导你如何通过 CPU 或 NPU 在 RevyOS 系统上运行 MobilenetV1 模型。 MobilenetV1 是一种高效的深度神经网络模型，广泛应用于移动设备和嵌入式系统优化。
...
适用于本教程的代码位于 `classification/mobilenetv1` 目录下。

## 模型获取

我们使用的模型来自 [mobilenet-caffe 模型仓库](https://github.com/shicai/MobileNet-Caffe)，可以通过以下命令下载 MobilenetV1 模型：
...
进入 `classification/mobilenetv1` 目录，执行以下命令：
...
在 `classification/mobilenetv1` 目录下，使用以下命令编译应用程序：
...
- -static：指定为静态链接
- -o mobilenetv1_example：指定生成名为 mobilenetv1_example 的可执行文件

编译命令正确执行完成后会在示例目录生成 `mobilenetv1_example` 文件
...
```shell-session
$ scp -r ../mobilenetv1/ debian@<开发板IP>:/home/debian/mobilenetv1/
```
...
进入开发板的 `/home/debian/mobilenetv1` 目录，确保你已经在 RevyOS 上安装了 SHL 库并配置了 `LD_LIBRARY_PATH`。然后运行以下命令：
