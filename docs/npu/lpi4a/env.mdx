...
Lichee Pi 4A 是 Sipeed 使用 TH1520 芯片推出的开发板，基本开发板开箱配置可以参考官网中的[开发板硬件配置](https://wiki.sipeed.com/hardware/zh/lichee/th1520/lpi4a/2_unbox.html)。开发板的操作系统选择 RevyOS 20250526 版本。你可以在[镜像刷写教程](../../Installation/intro.mdx)模块查看如何将 RevyOS 刷写进系统。
...
根据命令打印的位置（如上高亮行所示），将目录中的动态库复制到 `/usr/lib` 目录中。以上个代码块中输出的路径为例，可以使用如下的复制命令将其复制到目标目录中：
...
<TabItem value="npu">
```shell-session
$ sudo cp /home/debian/npu/.venv/lib/python3.11/site-packages/shl/install_nn2/th1520/lib/* /usr/lib/
```
</TabItem>
    <TabItem value="cpu">
```shell-session
$ sudo cp /home/debian/npu/.venv/lib/python3.11/site-packages/shl/install_nn2/c920/lib/* /usr/lib/
```
    </TabItem>
</Tabs>
...
C++ 接口的预编译好的二进制库已经保存在 [Github](https://github.com/zhangwm-pt/prebuilt_opencv)，更新[示例程序](#example-code)的子模块即可下载。
