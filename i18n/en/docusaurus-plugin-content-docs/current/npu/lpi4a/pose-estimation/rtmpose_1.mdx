...
It is recommended to use a virtual environment to isolate dependencies. You may use either `venv` or `conda` to create a virtual environment.
...
Download the SHL backend (execution providers), enabling onnxruntime to utilize SHL's high-performance optimization for C-SKY CPUs.
...
Navigate to the ONNX example directory in the source code and execute the example:
